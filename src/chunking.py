...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split *after* sentence-ending punctuation so that '.', '!' and '?'
        # remain part of the sentence.  The whitespace after the punctuation
        # is discarded when each sentence is stripped below.
        sentences = [
            sentence.strip()
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # If no separator is available, the only safe fallback is a hard
        # character split.  This also handles separators=[] gracefully.
        ...
        # The empty separator means "split anywhere" and is handled as the
        # final character-level fallback rather than with str.split("").
        if separator == "":
            return [
                current_text[index : index + self.chunk_size]
                for index in range(0, len(current_text), self.chunk_size)
        ...
        for piece in pieces:
            ...
            # A single piece is still too large, so move down to a smaller
            # separator before falling back to character splitting.
